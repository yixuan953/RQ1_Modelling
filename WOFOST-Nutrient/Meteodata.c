...
int GetMeteoData(Weather* meteo)
{
   ...
   data = malloc(lon_length * lat_length * sizeof(*data));
    ...
    // allocate SOC
    if ((retval = nc_inq_varid(ncid, "oc", &varid)))
       ERR(retval);
   ...
   data = malloc(lon_length * lat_length * sizeof(*data));
    ...
    if ((retval = nc_inq_varid(ncid, "clay", &varid)))
        ERR(retval);
    ...
    for (j = 0; j < lon_length; j++) {
        for (k = 0; k < lat_length; k++) {
            ...
            for (l = 0; l < time_length; l++) {
                ...
                N_total_dep[j][k][l] = roundz(N_total_dep[j][k][l], 3); // [kgN/ha]
                P_total_dep[j][k][l] = roundz(P_total_dep[j][k][l], 4); // [kgP/ha]

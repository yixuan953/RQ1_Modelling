...
void CalNutriAvail() {
    ...
    // The daily P availability is depend on the soil moisture [%] and P concentration [g/m3] in the soil solution
    NPC->st_P_avail = NPC->p_st.cP_inorg * Crop->st.RootDepth * WatBal->st.Moisture * 0.1; 
...
void CalNSurplus() {
     NPC->n_st.N_surplus = NPC->n_st.N_fert_input + NPC->n_st.N_decomp + NPC->n_st.N_dep - Crop->N_st.Uptake;                             
}
...
void CalDenitrification() {
    NPC->n_st.N_loss_N2 = NPC->n_st.N_surplus - NPC->n_st.N_loss_leach - NPC->n_st.N_loss_sub;
}
